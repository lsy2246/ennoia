...
export function MemoryPage() {
  ...
  const ownerOptions = [
    ...
    ...sessions.map((item) => ({ value: item.id, label: `Session · ${item.title}` })),
  ];
    ...
    <div className="resource-layout resource-layout--single">
      <section className="work-panel">
        <div className="page-heading">
          ...
          <p>{t("web.memory.description", "在这里按 owner、kind、稳定性和关键词查看记忆，并直接触发 recall 与 review。")}</p>
        </div>
        ...
        <div className="filter-bar">
          <input value={q} onChange={(event) => setQ(event.target.value)} placeholder={t("web.memory.search_placeholder", "搜索标题、摘要、正文或命名空间")} />
          ...
          <button type="button" className="secondary" onClick={() => void handleRecall()}>{t("web.memory.recall", "Recall")}</button>
        </div>
          ...
          {filtered.map((memory) => (
            <article key={memory.id} className="resource-card">
              ...
              {memory.sources.length > 0 ? (
                <pre className="log-view">{memory.sources.map((item) => `${item.kind}: ${item.reference}`).join("\n")}</pre>
              ) : null}
              <div className="button-row">
                <input value={reviewer} onChange={(event) => setReviewer(event.target.value)} placeholder={t("web.memory.reviewer", "reviewer")} />
                <button type="button" className="secondary" onClick={() => void handleReview(memory.id, "approve")}>approve</button>
                <button type="button" className="secondary" onClick={() => void handleReview(memory.id, "reject")}>reject</button>
                <button type="button" className="secondary" onClick={() => void handleReview(memory.id, "supersede")}>supersede</button>
                <button type="button" className="secondary" onClick={() => void handleReview(memory.id, "retire")}>retire</button>
              </div>
            </article>
          ))}
        </div>
