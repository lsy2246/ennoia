...
`cargo run -p ennoia-cli -- init` 会自动创建运行目录、基础配置、扩展与技能注册表、默认 API 上游渠道、数据库目录、日志目录，并同步未卸载的内置扩展与技能。初始化不会创建默认 Agent、`workspace/`、`agents/<agent_id>/`、`spaces/`、`policies/`、`global/`。
