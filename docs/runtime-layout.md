...
- `agents/<agent_id>/`：只有创建或运行对应 Agent 时创建，包含 `workspace/`、私有技能和产物。
...
`cargo run -p ennoia-cli -- init` 会自动创建运行目录、基础配置、扩展与技能注册表、默认 API 上游渠道、数据库目录、日志目录，并同步未卸载的内置扩展与技能。初始化不会创建默认 Agent、`agents/<agent_id>/`、`spaces/`、`policies/`、`global/`。

数据库结构来自源码内置基线：`assets/db.sql` 负责新库初始化，`assets/migrations/` 负责后续结构演进。系统在新库场景执行 `db.sql`，在已有库场景执行新增 migration。
