...
├─ skills/                     # 技能安装内容根目录
├─ data/
│  ├─ journal/                 # 系统内置原始会话记录，默认关闭，启用 journal 后懒创建
│  │  ├─ index/
│  │  │  └─ conversations.json
│  │  └─ conversations/
│  │     └─ <conversation_id>/
...
│  │        ├─ messages.jsonl
│  │        └─ events.jsonl
│  └─ extensions/              # 扩展私有运行数据，例如 memory/memory.db
└─ logs/
   ├─ server/
   ├─ agents/
   ├─ spaces/
...
- `data/journal/`：系统内置、文件机制的原始记录层，默认关闭。`journal.enabled = false` 时不读写该目录。
- `data/extensions/`：扩展私有运行数据根目录。核心不创建主业务数据库；`memory`、`workflow` 等扩展各自管理私有存储。
...
系统配置只走 TOML 文件；Journal 使用 `data/journal/` 文件；扩展数据结构由各扩展自己的 `data/extensions/` 与私有迁移机制管理。
