...
- `Server` 负责 API 路由、TOML 配置文件、前端日志、扩展代理与扩展运行时装配。
- `Extension Host` 负责扩展扫描、attach/detach、运行状态、诊断、贡献注册和扩展后端进程托管。
- `Session` 是内置扩展，源码物理位于 `builtins/extensions/session/`，负责 Conversation、Lane、Message 与 Handoff。
- `Memory` 是内置扩展，源码物理位于 `builtins/extensions/memory/`，负责记忆后端 API、私有数据库与前端页面。
...
- `Session` 是内置扩展：前端通过 `/api/ext/session/*` 访问会话能力。
- `Memory` 是内置扩展：前端通过扩展页挂载，后端通过 `/api/ext/{extension_id}/{*path}` 代理到扩展服务。
...
- 核心日志写入 `~/.ennoia/logs/`。
- 核心不提供主业务数据库，不管理会话事实、记忆、运行编排、任务、产物索引或调度队列。
- 扩展私有存储位于 `~/.ennoia/data/extensions/{extension_id}/`，例如 `session.db`、`memory.db` 和 `workflow.db`。
