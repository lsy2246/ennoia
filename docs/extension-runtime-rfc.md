...
4. Shell 拉取 registry 后挂载页面、面板、主题和语言包
...
- Shell 主要基于主动拉取快照，缺少对扩展图谱变化的订阅能力
...
- Shell 支持 snapshot + subscription 的动态挂载
...
Shell 应从“请求一次 registry 快照”演进为“快照 + 订阅”模型：
...
- `dev` 负责启动主服务、Shell dev server、watch graph 与 runner manager
...
- Shell 支持 snapshot + subscription
