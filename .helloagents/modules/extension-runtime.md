...
- 为 Provider、Behavior、Memory 和 Hook 贡献提供统一 Worker 调用边界
...
- Host 默认不注入 WASI/import，按贡献 `entry` / `handler` 前缀校验 RPC 方法，并使用 `runtime.memory_limit_mb` 与 `runtime.timeout_ms` 约束执行
...
- 依赖 `crates/server` 的能力路由和 HTTP API
