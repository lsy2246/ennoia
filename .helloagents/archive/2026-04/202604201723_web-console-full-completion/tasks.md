# 任务清单: web-console-full-completion

> **@status:** completed | 2026-04-20 17:43

```yaml
@feature: web-console-full-completion
@created: 2026-04-20
@status: completed
@mode: R3
```
...
- [√] 1.1 重构 `web/src/web/AppWeb.tsx` 与 `web/src/router.tsx`，建立正式控制台导航与页面分组 | depends_on: []
- [√] 1.2 新增控制台共享导航/页面头部/空态组件，统一列表页骨架 | depends_on: [1.1]
- [√] 1.3 调整 `web/src/styles.css`，让控制台结构、表格、卡片、状态标签和响应式布局匹配新壳层 | depends_on: [1.1,1.2]

### 2. i18n 与主题体系收口

- [√] 2.1 扩充 `web/packages/i18n/src/modules/*`，补齐导航、页面标题、动作按钮、空态与状态文案 key | depends_on: [1.1]
- [√] 2.2 重构 `web/src/stores/ui.ts` 与相关页面消费方式，移除页面散落硬编码 | depends_on: [2.1]
- [√] 2.3 调整 `web/packages/theme-runtime/src/index.ts` 与设置页逻辑，统一内置主题与扩展主题展示、切换和应用 | depends_on: [2.2]

### 3. 控制台能力补完

- [√] 3.1 重构 `web/src/pages/ConversationsPage.tsx`，补齐会话删除入口、刷新策略和更清晰的会话管理视图 | depends_on: [1.2,2.2]
...
- [√] 4.2 在 `crates/server` 中补日志读取接口，暴露 web 所需最小日志查询能力 | depends_on: [3.6]
...
- [√] 5.2 执行 `cargo fmt --all`、`cargo check --workspace`、`cargo test --workspace`、`bun run --cwd web typecheck`、`bun run --cwd web build` 并修正本轮问题 | depends_on: [5.1]
