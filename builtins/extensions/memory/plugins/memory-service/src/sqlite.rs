...
use crate::{
    AssembleRequest, EpisodeKind, EpisodeRecord, EpisodeRequest, MemoryError, MemoryKind,
    MemoryRecord, MemorySource, MemoryStatus, MemoryStore, RecallMode, RecallQuery, RecallResult,
    RememberReceipt, RememberRequest, ReviewAction, ReviewActionKind, ReviewReceipt, Stability,
};
use async_trait::async_trait;
use chrono::Utc;
use ennoia_kernel::{ContextFrame, ContextLayer, MemoryPolicy, OwnerKind, OwnerRef, RunContext};
...
impl MemoryStore for SqliteMemoryStore {
    ...
    async fn assemble_context(&self, req: AssembleRequest) -> Result<RunContext, MemoryError> {
        ...
        let mut view = RunContext::default();
